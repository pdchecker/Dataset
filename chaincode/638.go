...
// RangeQueryResult structure used for returning result of range query
...
func (t *SimpleChaincode) CreateAsset(ctx contractapi.TransactionContextInterface, assetID, group, inPSB string) error {
	...
	groupIndexKey, err := ctx.GetStub().CreateCompositeKey(index, []string{asset.Group, asset.ID})
	if err != nil {
		return err
	}
	...
	value := []byte{0x00}
	return ctx.GetStub().PutState(groupIndexKey, value)
}
...
func (t *SimpleChaincode) DeleteAsset(ctx contractapi.TransactionContextInterface, assetID string) error {
	...
	// Delete index entry
	return ctx.GetStub().DelState(groupIndexKey)
}

// TransferAsset transfers an asset by setting a new owner name on the asset
func (t *SimpleChaincode) TransferAsset(ctx contractapi.TransactionContextInterface, assetID, newPSB string) error {
	asset, err := t.ReadAsset(ctx, assetID)
	if err != nil {
		return err
	}
...
func (t *SimpleChaincode) GetAllItemHistory(ctx contractapi.TransactionContextInterface, itemID string) ([]RangeQueryResult, error) {
	// Execute a key range query on all keys starting with 'color'
	itemAssetResultsIterator, err := ctx.GetStub().GetStateByPartialCompositeKey(index, []string{itemID})
	if err != nil {
		return nil, err
	}
	defer itemAssetResultsIterator.Close()

	var records []RangeQueryResult
	for itemAssetResultsIterator.HasNext() {
		responseRange, err := itemAssetResultsIterator.Next()
		if err != nil {
			return nil, err
		}
...
func (t *SimpleChaincode) QueryAssetsByOwner(ctx contractapi.TransactionContextInterface, owner string) ([]*Asset, error) {
	queryString := fmt.Sprintf(`{"selector":{"docType":"asset","ToOwner":"%s"}}`, owner)
