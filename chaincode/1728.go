...
// distância percorrida em rodovia
// tempo em rodovia
// distância percorrida em vias urbanas
// tempo em vias urbanas

// tempo em horario de pico
// tempo de noite
...
type Car struct {
	...
	DistanciaPercorrida string `json:DistanciaPercorrida`
	TempoFuncionamento string `json:TempoFuncionamento`

	DistanciaPercorrida_Rodovia string `json:DistanciaPercorrida_Rodovia`
	TempoFuncionamento_Rodovia string `json:TempoFuncionamento_Rodovia`
	DistanciaPercorrida_Urbana string `json:DistanciaPercorrida_Urbana`
	TempoFuncionamento_Urbana string `json:TempoFuncionamento_Urbana`

	TempoHorarioPico string `json:TempoHorarioPico`
	TempoNoite string `json:TempoNoite`
...
func (s *SmartContract) initLedger(APIstub shim.ChaincodeStubInterface) sc.Response {
	cars := []Car{
		Car{Placa: "AAA-1111", FCV: "1", DistanciaPercorrida: "100", TempoFuncionamento: "3", DistanciaPercorrida_Rodovia: "70", TempoFuncionamento_Rodovia: "1", DistanciaPercorrida_Urbana: "30", TempoFuncionamento_Urbana: "2", TempoHorarioPico: "1", TempoNoite: "0"},
	}
...
func (s *SmartContract) createCar(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	var car = 	Car{Placa: args[0], FCV: args[1], DistanciaPercorrida: args[2], TempoFuncionamento: args[3], DistanciaPercorrida_Rodovia: args[4],
		 		TempoFuncionamento_Rodovia: args[5], DistanciaPercorrida_Urbana: args[6], TempoFuncionamento_Urbana: args[7], 
				TempoHorarioPico: args[8], TempoNoite: args[9]}

	carAsBytes, _ := json.Marshal(car)
	APIstub.PutState(args[0], carAsBytes)

	indexName := "placa~key"
	colorNameIndexKey, err := APIstub.CreateCompositeKey(indexName, []string{car.Placa, args[0]})
	if err != nil {
...
func (S *SmartContract) queryCarsByOwner(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	ownerAndIdResultIterator, err := APIstub.GetStateByPartialCompositeKey("placa~key", []string{owner})
	if err != nil {
		return shim.Error(err.Error())
	}
...
// 	for i < len(cars) {
// 		carAsBytes, _ := json.Marshal(cars[i])
// 		APIstub.PutState("AAA-"+strconv.Itoa(i), carAsBytes)
// 		i = i + 1
// 	}
