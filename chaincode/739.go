...
func TestReadAsset(t *testing.T) {
	...
	expectedAsset.Name = "name"
	expectedAsset.Price = 1000
...
func TestUpdateAsset(t *testing.T) {
	...
	err = c.UpdateAsset(ctx, "statebad", "new value", 1000)
	...
	err = c.UpdateAsset(ctx, "missingkey", "new value", 1000)
	assert.EqualError(t, err, "The asset missingkey does not exist", "should error when exists returns true when updating")

	err = c.UpdateAsset(ctx, "assetkey", "name2", 2000)
	expectedAsset := new(Asset)
	expectedAsset.Name = "name2"
	expectedAsset.Price = 2000
